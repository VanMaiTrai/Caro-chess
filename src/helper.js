...
const checkEdge = (maxRows, maxCells, rowIndex, cellIndex) => {
    return rowIndex >= 0 && rowIndex < maxRows && cellIndex >= 0 && cellIndex < maxCells;
}

const countValues = (data, rootRowIndex, rootCellIndex, maxRows, maxCells, vector) => {
    let valueCount = 0;
...
export const checkWin = (data, rootRowIndex, rootCellIndex, maxRows, maxCells) => {
    const vectors = [[1, 0], [0, 1], [1, 1], [-1, 1]];

    for (let i = 0; i < vectors.length; i++) {
        const vector = vectors[i];
        const revertVector = [-vector[0], -vector[1]];
        const countObj = countValues(data, rootRowIndex, rootCellIndex, maxRows, maxCells, vector);
        const revertCountObj = countValues(data, rootRowIndex, rootCellIndex, maxRows, maxCells, revertVector);

        console.log(countObj, revertCountObj);

        // win when row have 5 value, and does not block both two side
        if (countObj[0] + revertCountObj[0] - 1 === 5 && countObj[1] + revertCountObj[1] !== 2) {
            return true;
        }
    }
